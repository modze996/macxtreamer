...
use cache::{file_age_secs, image_cache_path};
...
enum SortKey { Name, Year, Rating, Genre }
...
enum Msg {
    ...
    ItemsLoaded { kind: String, items: Result<Vec<Item>, String> },
    EpisodesLoaded { series_id: String, episodes: Result<Vec<Episode>, String> },
    CoverLoaded { url: String, bytes: Vec<u8> },
    IndexBuilt { movies: usize, series: usize },
    SearchReady(Vec<Row>),
    IndexData { movies: Vec<Item>, series: Vec<Item> },
    PreloadSet { total: usize },
    PreloadTick,
...
impl MacXtreamer {
    fn new() -> Self {
        ...
        app.current_theme = if app.config.theme.is_empty() { "dark".into() } else { app.config.theme.clone() };
        if app.config.cover_ttl_days == 0 { app.config.cover_ttl_days = 7; }
        if app.config.cover_parallel == 0 { app.config.cover_parallel = 6; }
        if app.config.font_scale == 0.0 { app.config.font_scale = 1.15; }
    ...
    let kind_s = kind.to_string();
        tokio::spawn(async move {
            ...
            let _ = tx.send(Msg::ItemsLoaded { kind: kind_s, items: res.map_err(|e| e.to_string()) });
        });
    }
    ...
    let sid = series_id;
        tokio::spawn(async move {
            ...
            let _ = tx.send(Msg::EpisodesLoaded { series_id: sid, episodes: res.map_err(|e| e.to_string()) });
        });
    }
                ...
                if let Some(age) = file_age_secs(&path) {
                    if let Ok(mut f) = File::open(&path) {
                        ...
                        if f.read_to_end(&mut buf).is_ok() {
                            let _ = tx.send(Msg::CoverLoaded { url: url_s.clone(), bytes: buf });
                            served_any = true;
                            if age > ttl_secs { need_refresh = true; }
                        }
                    }
                }
            }
            if !served_any || need_refresh {
                if let Ok(resp) = reqwest::get(&url_s).await {
                    if let Ok(bytes) = resp.bytes().await {
                        ...
                        let _ = tx.send(Msg::CoverLoaded { url: url_s.clone(), bytes: data });
                        return;
                    }
                }
    ...
    fn spawn_build_index(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            for c in vod {
                if let Ok(items) = fetch_items(&cfg, "vod", &c.id).await { all_movies.extend(items); }
            }
            for c in ser {
                if let Ok(items) = fetch_items(&cfg, "series", &c.id).await { all_series.extend(items); }
            }
            // Dedup by id
            ...
            let _ = tx.send(Msg::IndexBuilt { movies: all_movies.len(), series: all_series.len() });
    ...
    fn spawn_preload_all(&mut self) {
        ...
        tokio::spawn(async move {
            ...
            let vod = fetch_categories(&cfg, "get_vod_categories").await.unwrap_or_default();
            let ser = fetch_categories(&cfg, "get_series_categories").await.unwrap_or_default();
            let live = fetch_categories(&cfg, "get_live_categories").await.unwrap_or_default();

            // Gesamtzahl: alle Kategorien zählen + geschätzte Item-Abfragen pro Kategorie
            let total_steps = vod.len() + ser.len() + live.len();
            let _ = tx.send(Msg::PreloadSet { total: total_steps.max(1) });
            ...
            for c in vod {
                if let Ok(items) = fetch_items(&cfg, "vod", &c.id).await {
                    for it in &items {
                        if let Some(cu) = &it.cover { cover_urls.push(cu.clone()); }
                    }
                }
                let _ = tx.send(Msg::PreloadTick);
            }
...
impl eframe::App for MacXtreamer {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if !self.theme_applied {
            match self.current_theme.as_str() { "light" => ctx.set_visuals(egui::Visuals::light()), _ => ctx.set_visuals(egui::Visuals::dark()) }
    ...
    for msg in self.rx.try_iter() {
            ...
            match msg {
                Msg::LiveCategories(res) => {
                    match res {
                        Ok(list) => {
                            ...
                            if let Some(ref wanted) = self.last_live_cat_id {
                                if let Some((i, cat)) = self.playlists.iter().enumerate().find(|(_, c)| &c.id == wanted) {
                    ...
                    if self.loading_done >= self.loading_total { self.is_loading = false; }
                },
                    ...
                    match res {
                        Ok(list) => {
                            ...
                            if let Some(ref wanted) = self.last_vod_cat_id {
                                if let Some((i, cat)) = self.vod_categories.iter().enumerate().find(|(_, c)| &c.id == wanted) {
                    ...
                    match res {
                        Ok(list) => {
                            ...
                            if let Some(ref wanted) = self.last_series_cat_id {
                                if let Some((i, cat)) = self.series_categories.iter().enumerate().find(|(_, c)| &c.id == wanted) {
                        ...
                        Ok(items) => {
                            ...
                            for it in items {
                                ...
                                self.content_rows.push(Row {
                                    ...
                                    id: it.id,
                                    info: info.to_string(),
                                    container_extension: if info == "Movie" && !it.container_extension.is_empty() { Some(it.container_extension) } else { None },
                ...
                Msg::EpisodesLoaded { series_id: _sid, episodes } => match episodes {
                    Ok(eps) => {
                        self.content_rows.clear();
                        for ep in eps {
                            self.content_rows.push(Row {
                ...
                Msg::IndexBuilt { movies: _m, series: _s } => {
    ...
    egui::TopBottomPanel::top("top").resizable(true).show_separator_line(true).default_height(win_h / 3.0).show(ctx, |ui| {
            // Kopfzeile mit Aktionen und Suche
            ui.horizontal(|ui| {
                ui.heading("MacXtreamer");
                if ui.button("Reload").clicked() {
                    // Reset loading and reload categories; item reload happens when categories arrive
                    self.is_loading = true;
                    self.loading_total = 3;
                    self.loading_done = 0;
                ...
                ui.add_space(6.0);
                ui.small("Player command supports the placeholder URL");
                // Theme Toggle
                egui::ComboBox::from_label("")
                    .selected_text(if self.current_theme == "light" { "Light" } else { "Dark" })
                    .show_ui(ui, |ui| {
                        if ui.selectable_label(self.current_theme == "dark", "Dark").clicked() { self.current_theme = "dark".into(); self.theme_applied = false; }
                        if ui.selectable_label(self.current_theme == "light", "Light").clicked() { self.current_theme = "light".into(); self.theme_applied = false; }
                    });
                ui.separator();
                ui.label("Cover height");
                ui.add(egui::Slider::new(&mut self.cover_height, 40.0..=120.0).step_by(2.0));
                if self.is_loading {
                    let pct = if self.loading_total > 0 { (self.loading_done * 100 / self.loading_total).min(100) } else { 0 };
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    let resp = egui::TextEdit::singleline(&mut self.search_text)
                        ...
                        .lock_focus(true)
                        .show(ui);
                    if resp.response.lost_focus() && resp.response.ctx.input(|i| i.key_pressed(egui::Key::Enter)) {
                        self.start_search();
                        resp.response.request_focus();
                    }
                });
                if self.indexing { ui.label("Indexing…"); }
            });
            ...
            ui.columns(3, |cols| {
                ...
                egui::ScrollArea::vertical().id_source("live_list").show(&mut cols[0], |ui| {
                    for (i, c) in self.playlists.clone().into_iter().enumerate() {
                        if ui.selectable_label(self.selected_playlist == Some(i), &c.name).clicked() {
                            self.selected_playlist = Some(i);
                            self.is_loading = true;
                            self.loading_total = 1;
                            self.loading_done = 0;
                ...
                egui::ScrollArea::vertical().id_source("vod_list").show(&mut cols[1], |ui| {
                    for (i, c) in self.vod_categories.clone().into_iter().enumerate() {
                        if ui.selectable_label(self.selected_vod == Some(i), &c.name).clicked() {
                            self.selected_vod = Some(i);
                            self.is_loading = true;
                            self.loading_total = 1;
                            self.loading_done = 0;
                ...
                egui::ScrollArea::vertical().id_source("series_list").show(&mut cols[2], |ui| {
                    for (i, c) in self.series_categories.clone().into_iter().enumerate() {
                        if ui.selectable_label(self.selected_series == Some(i), &c.name).clicked() {
                            self.selected_series = Some(i);
                            self.is_loading = true;
                            self.loading_total = 1;
                            self.loading_done = 0;
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if let Some(key) = self.sort_key {
                match key {
                    ...
                    SortKey::Year => {
                        fn parse_year(y: &Option<String>) -> i32 { y.as_deref().and_then(|s| s.parse::<i32>().ok()).unwrap_or(0) }
                    ...
                    SortKey::Genre => {
                        rows.sort_by(|a,b| a.genre.clone().unwrap_or_default().to_lowercase().cmp(&b.genre.clone().unwrap_or_default().to_lowercase()));
                    }
                }
                if !self.sort_asc { rows.reverse(); }
            }
            ...
            let header_h = 22.0;
                ...
                .header(header_h, |mut header| {
                    header.col(|ui| { ui.strong("Cover"); });
                    header.col(|ui| {
                        let selected = self.sort_key == Some(SortKey::Name);
                        let label = if selected { format!("Name {}", if self.sort_asc {"▲"} else {"▼"}) } else { "Name".to_string() };
                    ...
                    header.col(|ui| {
                        ...
                        let label = if selected { format!("Year {}", if self.sort_asc {"▲"} else {"▼"}) } else { "Year".to_string() };
                        if ui.small_button(label).clicked() {
                    ...
                    header.col(|ui| {
                        ...
                        let label = if selected { format!("Rating {}", if self.sort_asc {"▲"} else {"▼"}) } else { "Rating".to_string() };
                        if ui.small_button(label).clicked() {
                    ...
                    header.col(|ui| {
                        ...
                        let label = if selected { format!("Genre {}", if self.sort_asc {"▲"} else {"▼"}) } else { "Genre".to_string() };
                        if ui.small_button(label).clicked() {
                ...
                .body(|body| {
                    ...
                    body.rows(row_h, row_count, |i, mut row| {
                        let r = &rows[i];
                        let url = r.stream_url.clone().unwrap_or_else(|| build_url_by_type(&self.config, &r.id, &r.info, r.container_extension.as_deref()));
                        ...
                        row.col(|ui| {
                            if let Some(cu) = &r.cover_url {
                                if let Some(tex) = self.textures.get(cu) {
                                    ...
                                    let (rect, _resp) = ui.allocate_exact_size(egui::vec2(cover_w, self.cover_height), egui::Sense::hover());
                        ...
                        row.col(|ui| { ui.label(&r.id); });
                        row.col(|ui| { ui.label(&r.info); });
                        row.col(|ui| { ui.label(r.year.clone().unwrap_or_default()); });
                        row.col(|ui| { ui.label(r.rating_5based.map(|v| format!("{:.1}", v)).unwrap_or_default()); });
                        row.col(|ui| { ui.label(r.genre.clone().unwrap_or_default()); });
                        row.col(|ui| {
                            ui.horizontal_wrapped(|ui| {
                                if r.info == "Series" {
                                    ...
                                    if ui.small_button("Play").clicked() {
                                        ...
                                        let rec = RecentItem { id: r.id.clone(), name: r.name.clone(), info: r.info.clone(), stream_url: url.clone() };
                                        add_to_recently(&rec);
                                        self.recently = load_recently_played();
                                    }
                                    if ui.small_button("Copy").clicked() { ui.output_mut(|o| o.copied_text = url.clone()); }
                                    if ui.small_button("Fav").clicked() {
                                        toggle_favorite(&FavItem { id: r.id.clone(), info: r.info.clone(), name: r.name.clone(), stream_url: Some(url.clone()) });
    ...
    // (kein zweites Bottom-Panel)
                                ...
                                if self.recently.is_empty() {
                                    ...
                                    for it in &self.recently {
                                        if ui.button(format!("{} ({})", it.name, it.info)).clicked() {
                                            let _ = start_player(&self.config, &it.stream_url);
                                        }
                                    }
                                }
                            });
        ...
        if self.pending_save_config {
            ...
            let permits = if self.config.cover_parallel == 0 { 6 } else { self.config.cover_parallel } as usize;
